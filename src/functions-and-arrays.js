...
function findLongestWord(arrayOfWords) {
  if (arrayOfWords.length === 0) {
    return null;
  } //if(!arrayOfWords.length)- if falsy value we need to negate using !
...
function averageNumbers(avarageResult) {
  if (avarageResult.length === 0) {
    return null;
  }
  let average = 0;
  for (let i = 0; i < avarageResult.length; i++) {
    average += avarageResult[i];
  }
  return average / avarageResult.length;
}
